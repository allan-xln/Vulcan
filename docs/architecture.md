...
- `apps/web`: tenant-facing control plane for analytics, alerts, audit views and explainability.
- `services/ai-api`: explanation and scoring API over structured inputs.
- `services/ingestion-gateway`: reserved for signed telemetry ingestion and validation.
- `services/agent-collector`: reserved Windows-first collector layout with osquery-based observability only.
- `packages/domain`: shared domain types and invariants.
