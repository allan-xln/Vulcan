...
from app.normalization_rules import NormalizedOperationalEvent, RawOperationalEventRecord
...
class NormalizationRepository(Protocol):
    def start_run(self, tenant_id: UUID) -> UUID: ...
    def fetch_pending_raw_events(self, tenant_id: UUID, limit: int) -> list[RawOperationalEventRecord]: ...
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    select
                      ...
                      r.received_at,
                      r.event_payload
                    from public.raw_operational_event_intake r
                    left join public.normalized_operational_events n
                      on n.raw_operational_event_intake_id = r.id
                    where r.ingestion_status = 'accepted'
                      and n.id is null
                      and r.tenant_id = %(tenant_id)s::uuid
                ...
                for event in events:
                    cursor.execute(
                        ...
                        insert into public.normalized_operational_events (
                          tenant_id,
                          raw_operational_event_intake_id,
                          source_event_id,
                          workstation_id,
                        ...
                        values (
                          %(tenant_id)s,
                          %(raw_operational_event_intake_id)s,
                          %(source_event_id)s,
                          %(workstation_id)s,
                        ...
                        on conflict (raw_operational_event_intake_id) do nothing
                        """,
                        {
                            "tenant_id": event.tenant_id,
                            "raw_operational_event_intake_id": event.raw_event_id,
