...
class DailyMetricRepository(Protocol):
    def start_run(self, tenant_id: UUID, metric_date: date | None) -> UUID: ...
    def fetch_session_slices(self, tenant_id: UUID, metric_date: date | None) -> list[SessionSliceRecord]: ...
    def fetch_idle_windows(self, tenant_id: UUID, metric_date: date | None) -> list[IdleWindowRecord]: ...
    def fetch_application_usage_facts(self, tenant_id: UUID, metric_date: date | None) -> list[ApplicationUsageRecord]: ...
        ...
        with self._connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    where iw.tenant_id = %(tenant_id)s::uuid
        ...
        with self._connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    where auf.tenant_id = %(tenant_id)s::uuid
