...
from app.schemas import OperationalEventBatchRequest, OperationalEvent
...
class IngestionRepository(Protocol):
    ...
    def validate_event_sources(self, tenant_id: UUID, events: list[OperationalEvent]) -> None: ...
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    where id = %(key_id)s
                      ...
                      and scopes @> array['operational_events:ingest']::text[]
                    """,
        ...
        with self._connect() as conn:
            with conn.cursor() as cursor:
                for event in batch.events:
                    ...
                    cursor.execute(
                        """
                        insert into public.raw_operational_event_intake (
                          tenant_id,
                    ...
                    cursor.execute(
                        """
                        select id
                        from public.raw_operational_event_intake
                        where tenant_id = %(tenant_id)s
