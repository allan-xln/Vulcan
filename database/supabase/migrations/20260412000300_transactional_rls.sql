...
alter table public.operational_event_policies enable row level security;
...
drop policy if exists operational_event_policies_read_member on public.operational_event_policies;
create policy operational_event_policies_read_member
on public.operational_event_policies
for select
to authenticated
using (public.is_tenant_member(tenant_id));

drop policy if exists operational_event_policies_manage_admin on public.operational_event_policies;
create policy operational_event_policies_manage_admin
on public.operational_event_policies
for all
to authenticated
using (public.can_manage_tenant(tenant_id))
