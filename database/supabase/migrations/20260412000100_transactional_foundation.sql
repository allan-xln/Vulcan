...
create table if not exists public.operational_event_policies (
  id uuid primary key default gen_random_uuid(),
  tenant_id uuid not null references public.tenants (id) on delete cascade,
...
create table if not exists public.tenant_usage (
  ...
  operational_event_count bigint not null default 0 check (operational_event_count >= 0),
...
create index if not exists idx_operational_event_policies_tenant_id on public.operational_event_policies (tenant_id);
...
grant select, insert, update, delete on public.operational_event_policies to authenticated;
