...
create table if not exists public.ingestion_api_keys (
  ...
  scopes text[] not null default array['telemetry:ingest']::text[],
...
create table if not exists public.raw_telemetry_intake (
  id uuid primary key default gen_random_uuid(),
  tenant_id uuid not null references public.tenants (id) on delete cascade,
  ...
  duplicate_of_id uuid references public.raw_telemetry_intake (id) on delete set null,
...
create index if not exists idx_raw_telemetry_intake_tenant_received_at on public.raw_telemetry_intake (tenant_id, received_at desc);
create index if not exists idx_raw_telemetry_intake_status on public.raw_telemetry_intake (ingestion_status, received_at);
create index if not exists idx_raw_telemetry_intake_request_id on public.raw_telemetry_intake (request_id);
create index if not exists idx_raw_telemetry_intake_workstation on public.raw_telemetry_intake (tenant_id, workstation_id, occurred_at desc);
...
grant all privileges on public.raw_telemetry_intake to service_role;

alter table public.ingestion_api_keys enable row level security;
alter table public.raw_telemetry_intake enable row level security;
...
drop trigger if exists trg_raw_telemetry_intake_set_audit_fields on public.raw_telemetry_intake;
create trigger trg_raw_telemetry_intake_set_audit_fields
before insert or update on public.raw_telemetry_intake
for each row execute function app_private.set_audit_fields();
