...
- The local run path bootstraps a minimal compatibility layer for plain Postgres so Phase 2 can be validated without the full Supabase stack.
...
- `org_closure` is rebuilt by triggers when nodes or edges change. That keeps the Phase 2 implementation simple and correct, while leaving room for a more incremental maintenance strategy later.
