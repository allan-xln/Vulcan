...
from app.schemas import TelemetryBatchRequest, TelemetryBatchResponse
...
def get_service(repository: IngestionRepository = Depends(get_repository)) -> TelemetryIngestionService:
    return TelemetryIngestionService(repository=repository)
...
@app.post("/v1/telemetry/events", response_model=TelemetryBatchResponse)
def ingest_events(
    batch: TelemetryBatchRequest,
    request: Request,
    service: TelemetryIngestionService = Depends(get_service),
