...
from app.normalization_rules import NormalizedOperationalEvent, RawTelemetryRecord, normalize_raw_event
...
class FakeNormalizationRepository(NormalizationRepository):
    def __init__(self, raw_events: list[RawTelemetryRecord]) -> None:
...
def make_raw_event(event_type: str, payload: dict, source_event_id: str) -> RawTelemetryRecord:
    return RawTelemetryRecord(
        ...
        schema_version="2026-04-telemetry.v1",
...
def test_normalization_service_is_replay_safe() -> None:
    ...
    first_result = service.run(NormalizationRequest(batch_limit=10))
    second_result = service.run(NormalizationRequest(batch_limit=10))
